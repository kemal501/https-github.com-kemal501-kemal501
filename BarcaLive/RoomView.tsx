...
export default function RoomView({ room, isHost, onLeave }: RoomViewProps) {
  ...
  const [layoutMode, setLayoutMode] = React.useState<'classic' | 'immersive'>('immersive');
  const [userCoins, setUserCoins] = React.useState<number>(25000);
  const [speakingSeats, setSpeakingSeats] = React.useState<Record<number, boolean>>(() => ({
    0: true,
  }));
  ...
  React.useEffect(() => {
    ...
    const unsub = onSnapshot(userRef, (snapshot) => {
      ...
      console.warn("Could not load user coins dynamically:", error);
    });
    return () => unsub();
  }, []);
        ...
        <div className="relative w-full h-full flex flex-col items-center justify-center bg-[#111] overflow-hidden">
          <style>{`
            ...
            .speaking-glow {
              ...
              border: 3px solid #00ffd0 !important;
          ...
          {/* Full Screen Ambient Blur Background */}
          <img 
            className="fixed inset-0 w-full h-full object-cover select-none z-0 filter blur-[3px] scale-105 pointer-events-none" 
          ...
          {/* Live Badge */}
          <div className="absolute top-[18px] left-1/2 -translate-x-1/2 bg-red-600 px-[16px] py-[6px] rounded-full text-[10px] font-black uppercase tracking-widest text-white shadow-lg shadow-red-600/20 animate-pulse z-[20] flex items-center gap-1.5 border border-red-500/20">
            <span className="w-1.5 h-1.5 rounded-full bg-white animate-ping" />
            🔴 LIVE
          </div>

          {/* Phone Frame Mockup Container for Desktop, Fullscreen on Mobile */}
          <div className="relative w-full max-w-md h-full md:max-h-[850px] md:h-[820px] md:rounded-[3rem] md:border-[10px] md:border-zinc-800 md:shadow-2xl md:ring-[8px] md:ring-zinc-900 overflow-hidden bg-zinc-950 flex flex-col z-[10] border border-white/5">
            
            {/* Blurred Background inside viewport */}
            <img 
              className="absolute inset-0 w-full h-full object-cover select-none z-0 filter blur-[2px] scale-105 pointer-events-none" 
              src="https://images.unsplash.com/photo-1516280440614-37939bbacd81?q=80&w=1400&auto=format&fit=crop"
              alt="Room background inside"
              referrerPolicy="no-referrer"
            />
            <div className="absolute inset-0 bg-black/45 z-[5]" />

            {/* Immersive View Header / Top Bar */}
            <div className="absolute top-[15px] left-[15px] right-[15px] flex justify-between items-center z-[20]">
              <div className="flex items-center gap-2.5 bg-black/40 px-[12px] py-[8px] rounded-[24px] backdrop-blur-[10px] border border-white/5 text-left">
                <button 
                  type="button"
                  onClick={() => setShowHostCard(!showHostCard)}
                  className="w-[42px] h-[42px] rounded-full overflow-hidden border-2 border-white cursor-pointer active:scale-95 transition-transform"
                >
                  <img src={`https://i.pravatar.cc/150?u=${room.host}`} alt="Host" className="w-full h-full object-cover" />
                </button>
                <div className="text-left">
                  <h3 className="text-white text-[13px] font-black tracking-tight uppercase truncate max-w-[100px]">{room.host}</h3>
                  <p className="text-zinc-300 text-[9px] font-bold tracking-widest uppercase opacity-90 mt-0.5">ID: {room.id.slice(0, 8)}</p>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <button 
                  ...
                  className="bg-amber-400 text-black font-black text-[9px] uppercase tracking-widest px-3.5 py-2.5 rounded-full hover:scale-105 active:scale-95 transition-all shadow-md flex items-center gap-1 cursor-pointer"
                >
                ...
                <button 
                  type="button"
                  onClick={onLeave}
                  className="w-[38px] h-[38px] rounded-full bg-red-600/80 hover:bg-red-600 flex justify-center items-center text-[15px] text-white hover:scale-110 active:scale-90 transition-all cursor-pointer z-[20] shadow-md shadow-red-950/40"
                  title="Leave Room"
                >
                  ✖
                </button>
            ...
            {/* Coin balance bar */}
            <div className="absolute top-[80px] left-[15px] flex gap-[10px] z-[20]">
              <div className="bg-gradient-to-r from-amber-400 to-orange-500 text-black px-4 py-2 rounded-full text-[10px] font-black flex items-center gap-1.5 shadow-md shadow-orange-500/10">
                🪙 {userCoins.toLocaleString()}
              </div>
              <div className="bg-black/45 border border-white/5 backdrop-blur-md text-white px-4 py-2 rounded-full text-[10px] font-black flex items-center gap-1.5 shadow-md">
                🎮 320
              </div>
            </div>

            {/* Immersive Seat Grid */}
            <div className="absolute top-[145px] left-1/2 -translate-x-1/2 w-[92%] grid grid-cols-3 gap-x-5 gap-y-8 justify-items-center z-[20]">
              {seats.slice(0, 9).map((seat, index) => {
                const isVip = index < 3;
                const isSpeaking = speakingSeats[seat.id];
                ...
                return (
                  <div 
                    ...
                    onClick={() => {
                      if (isOccupied) {
                        setSpeakingSeats(prev => ({ ...prev, [seat.id]: !prev[seat.id] }));
                    ...
                    className={cn(
                      "w-[76px] h-[76px] md:w-[82px] md:h-[82px] rounded-full flex justify-center items-center relative border-2 backdrop-blur-md transition-all duration-300 cursor-pointer shadow-lg",
                      isOccupied 
                        ? (isSpeaking ? "speaking-glow border-transparent" : "bg-black/40 border-amber-400/30 hover:border-amber-400")
                        : (seat.isLocked ? "bg-red-950/20 border-red-900/40" : "bg-white/10 border-white/15 hover:bg-white/20 hover:border-white/30")
                    )}
                  >
                    {isVip && (
                      <span className="absolute -top-3 text-[20px] z-20 animate-bounce select-none pointer-events-none" title="VIP Crown">
                        👑
                      </span>
                    )}
                    ...
                    {isOccupied ? (
                      <img 
                        src={seat.user?.avatar || `https://i.pravatar.cc/150?img=${10 + seat.id}`} 
                        alt="Occupant" 
                        className="w-full h-full object-cover rounded-full border-2 border-transparent" 
                        referrerPolicy="no-referrer"
                      />
                    ) : (
                      <div className="text-[22px] opacity-80 select-none">
                        {seat.isLocked ? "🔒" : "🎤"}
                      </div>
                    )}

                    <div className="absolute -bottom-5 left-1/2 -translate-x-1/2 text-center w-full">
                      <span className="bg-black/70 backdrop-blur-md px-1.5 py-0.5 rounded-full text-[8.5px] font-black tracking-wide text-zinc-200 shadow-md border border-white/5 truncate max-w-[70px] block mx-auto uppercase">
                        {isOccupied ? seat.user?.name : `Mic ${index + 1}`}
                      </span>
                    </div>
                  </div>
                );
            ...
            {/* Room Notice Rules Board */}
            <div className="absolute bottom-[230px] left-1/2 -translate-x-1/2 w-[90%] bg-black/60 p-4 rounded-[20px] backdrop-blur-[10px] text-[11px] font-semibold leading-relaxed border border-white/5 text-zinc-300 z-[20] text-left">
              <b className="text-amber-400 tracking-wider text-[11px] uppercase mb-1 block font-black">Barca-live Notice</b>
              1. Welcome to Barca-live Room! Keep matches clean.<br/>
              2. Earn continuous gold coins by staying on active mics.<br/>
              3. Room Admins retain full broadcast management rights.
            </div>

            {/* Scrolling Chat messages list overlay */}
            <div className="absolute bottom-[150px] left-[15px] right-[15px] max-h-[75px] overflow-y-auto z-[20] flex flex-col gap-1.5 no-scrollbar scroll-smooth">
              {messages.length === 0 ? (
                <>
                  <div className="bg-black/45 px-3 py-1.5 rounded-[15px] text-[11px] font-bold text-zinc-300 border border-white/5 backdrop-blur-md w-max max-w-full text-left">
                    🎉 Welcome to <b>{room.title}</b> live lounge!
                  </div>
                  <div className="bg-black/45 px-3 py-1.5 rounded-[15px] text-[11px] font-bold text-zinc-300 border border-white/5 backdrop-blur-md w-max max-w-full text-left">
                    📢 Please adhere to the rules in Barca rooms!
                  </div>
                </>
              ) : (
                messages.filter(msg => !blockedUsers.includes(msg.senderName)).slice(-5).map((msg) => (
                  <div 
                    key={msg.id}
                    className="bg-black/60 px-[14px] py-[8px] rounded-[15px] text-[11px] font-bold text-zinc-200 border border-white/5 backdrop-blur-[10px] w-max max-w-full break-words text-left shadow-sm"
                  >
                    <strong className="text-amber-400 uppercase font-black text-[9px] mr-1">
                      {msg.senderName}:
                    </strong>
                    {msg.text}
                  </div>
            ...
            {/* Float Gift Button Trigger */}
            <div 
              onClick={() => {
                setIsPurchaseMode(false);
                setShowGifts(true);
              }}
              className="absolute right-[15px] bottom-[95px] w-[46px] h-[46px] rounded-full bg-gradient-to-br from-pink-500 to-indigo-600 flex justify-center items-center text-[20px] shadow-[0_0_15px_rgba(255,255,255,0.25)] hover:shadow-[0_0_20px_rgba(255,255,255,0.4)] active:scale-95 hover:scale-105 transition-all z-[30] border border-white/20 cursor-pointer"
              title="Send Gift"
            >
              🎁
            </div>
            ...
            <form 
              ...
              className="absolute bottom-[95px] left-[15px] right-[75px] flex gap-[8px] items-center z-[20]"
            >
              <input 
                className="flex-grow h-[42px] border border-white/10 outline-none rounded-full px-5 bg-white/10 text-white placeholder-zinc-400 text-[11px] backdrop-blur-md focus:border-amber-400/60 focus:bg-white/15 transition-all"
                placeholder="Say hello to Barca..."
                value={chatMessage}
                onChange={(e) => setChatMessage(e.target.value)}
              />
              <button 
                type="submit"
                className="w-[42px] h-[42px] rounded-full bg-[#00d5b5] text-black hover:bg-[#00f3cf] active:scale-90 flex justify-center items-center text-[15px] cursor-pointer shadow-lg shadow-[#00d5b5]/20 transition-all font-bold"
              >
                ➤
              </button>
            </form>

            {/* Immersive Bottom Menu list */}
            <div className="absolute bottom-0 left-0 right-0 bg-black/85 backdrop-blur-[12px] p-[10px] pb-[15px] pt-[12px] border-t border-zinc-900 rounded-t-[25px] md:rounded-t-[30px] z-[30]">
              <div className="grid grid-cols-8 gap-y-3 gap-x-1 text-center">
                
                {/* Mic toggle */}
                <div onClick={() => {
                  const nextMute = !isMuted;
                  setIsMuted(nextMute);
                  toggleMyMute(nextMute);
                }} className="flex flex-col items-center gap-1.5 cursor-pointer group">
                  <div className={cn("w-[34px] h-[34px] rounded-[10px] flex justify-center items-center text-[15px] transition-all", isMuted ? "bg-red-500 text-white" : "bg-white/10 hover:bg-white/20")}>
                    {isMuted ? "🔇" : "🎤"}
                  </div>
                  <span className="text-[7.5px] font-black text-zinc-400 group-hover:text-white uppercase tracking-wider">Mic</span>
                </div>

                {/* Music toggle */}
                <div onClick={() => {
                  const nextMusic = !isBgmEnabled;
                  setIsBgmEnabled(nextMusic);
                  handleUpdateRoomSettings({ 'settings.isBgmEnabled': nextMusic });
                }} className="flex flex-col items-center gap-1.5 cursor-pointer group">
                  <div className={cn("w-[34px] h-[34px] rounded-[10px] flex justify-center items-center text-[15px] transition-all", isBgmEnabled ? "bg-emerald-600 text-white animate-pulse" : "bg-white/10 hover:bg-white/20")}>
                    🎵
                  </div>
                  <span className="text-[7.5px] font-black text-zinc-400 group-hover:text-white uppercase tracking-wider">Music</span>
                </div>
                ...
                <div onClick={() => {
                  if (isHost) {
                    ...
                    alert("Only Host can adjust direct room streaming parameters.");
                  }
                }} className="flex flex-col items-center gap-1.5 cursor-pointer group">
                  <div className="w-[34px] h-[34px] rounded-[10px] bg-white/10 hover:bg-white/20 flex justify-center items-center text-[15px] transition-all">
                    ⚙
                  </div>
                  <span className="text-[7.5px] font-black text-zinc-400 group-hover:text-white uppercase tracking-wider">Settings</span>
                </div>

                {/* Bag */}
                <div onClick={() => setShowTaskAgent(true)} className="flex flex-col items-center gap-1.5 cursor-pointer group">
                  <div className="w-[34px] h-[34px] rounded-[10px] bg-white/10 hover:bg-white/20 flex justify-center items-center text-[15px] transition-all">
                    🎒
                  </div>
                  <span className="text-[7.5px] font-black text-zinc-400 group-hover:text-white uppercase tracking-wider">Bag</span>
                </div>

                {/* Recharge */}
                <div onClick={() => setShowResellerCoin(true)} className="flex flex-col items-center gap-1.5 cursor-pointer group">
                  <div className="w-[34px] h-[34px] rounded-[10px] bg-white/10 hover:bg-white/20 flex justify-center items-center text-[15px] transition-all">
                    🪙
                  </div>
                  <span className="text-[7.5px] font-black text-zinc-400 group-hover:text-white uppercase tracking-wider font-mono">Resell</span>
                </div>

                {/* Games */}
                <div onClick={() => setShowMinigames(true)} className="flex flex-col items-center gap-1.5 cursor-pointer group">
                  <div className="w-[34px] h-[34px] rounded-[10px] bg-white/10 hover:bg-white/20 flex justify-center items-center text-[15px] transition-all">
                    🎮
                  </div>
                  <span className="text-[7.5px] font-black text-zinc-400 group-hover:text-white uppercase tracking-wider">Games</span>
                </div>

                {/* Sound FX */}
                <div onClick={() => setShowVoiceFX(true)} className="flex flex-col items-center gap-1.5 cursor-pointer group">
                  <div className={cn("w-[34px] h-[34px] rounded-[10px] flex justify-center items-center text-[15px] transition-all", selectedVoiceEffect !== 'none' ? "bg-purple-600 text-white" : "bg-white/10 hover:bg-white/20")}>
                    🎉
                  </div>
                  <span className="text-[7.5px] font-black text-zinc-400 group-hover:text-white uppercase tracking-wider">FX</span>
                </div>
                ...
                <div onClick={() => {
                  if (isHost) {
                    setShowHostConsole(true);
                    setActiveConsoleTab('permissions');
                  } else {
                    alert("Admin console is secured under Barca Host Authority.");
                  }
                }} className="flex flex-col items-center gap-1.5 cursor-pointer group">
                  <div className="w-[34px] h-[34px] rounded-[10px] bg-white/10 hover:bg-white/20 flex justify-center items-center text-[15px] transition-all">
                    🛡
                  </div>
                  <span className="text-[7.5px] font-black text-zinc-400 group-hover:text-white uppercase tracking-wider">Admin</span>
                </div>
