...
import { Coins, CheckCircle2, Flame, Trophy, Play, Waves, MessageSquare, Loader2, Sparkles, Check, X } from 'lucide-react';
import { db, auth } from '../lib/firebase';
import { doc, onSnapshot, setDoc, updateDoc, increment, getDoc, serverTimestamp } from 'firebase/firestore';
                ...
                {isClaimed ? (
                  ...
                  activeConfirmId === mission.id ? (
                    <div className="space-y-2.5 p-3.5 bg-zinc-950/60 border border-amber-500/20 rounded-2xl">
                      <p className="text-[10px] uppercase tracking-wider font-extrabold text-amber-300 text-center animate-pulse flex items-center justify-center gap-1">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-400 hover:to-emerald-500 text-white font-black py-2.5 rounded-xl uppercase tracking-widest text-[8.5px] transition-all cursor-pointer flex items-center justify-center gap-1 shadow-md shadow-emerald-500/10"
                        >
                    ...
                    <button
                      ...
                      disabled={claimingId !== null}
                      className="w-full bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-black font-black py-3 rounded-xl uppercase tracking-widest text-[9.5px] transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-lg shadow-amber-400/10 hover:scale-[1.01]"
                    >
