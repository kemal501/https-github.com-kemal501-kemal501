...
export default function UserTasks() {
  ...
  return (
    <div className="space-y-8 p-6" id="user-tasks">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
          {['all', 'available', 'in-progress', 'completed', 'locked'].map(f => (
            <button
              ...
              className={cn(
                "px-4 py-2 rounded-full border transition-all text-[9px] font-black uppercase tracking-widest flex-shrink-0 cursor-pointer",
                filter === f 
            ...
            !isFaceVerified ? (
              ...
              >
                <div className="space-y-1.5">
                  ...
                  <p className="text-zinc-500 text-[10px] font-medium leading-relaxed max-w-[420px] font-sans">
                ...
                <button
                  ...
                  className="bg-amber-400 hover:bg-amber-300 text-black font-black uppercase text-[10px] tracking-widest px-6 py-4 rounded-2xl shadow-lg shadow-amber-400/10 active:scale-95 transition-all self-start md:self-center flex items-center gap-2 cursor-pointer border-0"
                >
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <h3 className="text-white font-black italic uppercase text-sm tracking-tight flex items-center gap-1.5">
                      ...
                      <Star className="w-4 h-4 text-purple-400 animate-pulse fill-purple-400/20" />
                    </h3>
                    <span className={cn(
                  ...
                  <p className="text-zinc-500 text-[10px] font-medium leading-relaxed max-w-[280px] font-sans">
                ...
                <div className="text-right">
                  ...
                  <span className="text-zinc-600 text-[8px] font-black uppercase tracking-widest font-mono">COINS</span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <div className="flex justify-between text-[9px] font-black uppercase text-zinc-500 tracking-widest font-sans">
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <div className="p-4 bg-black/30 border border-zinc-900 rounded-2xl flex flex-col justify-between text-left">
                    <div>
                      <span className="text-[8px] font-black text-purple-400 uppercase tracking-widest font-sans animate-pulse">1 Hour (Half)</span>
                      <p className="text-[10px] font-medium text-zinc-400 mt-1 uppercase font-mono">+{isNewUser ? "10,000,000" : "5,000,000"} Coins</p>
                    </div>
                    <div className="mt-3">
                      {sitRewardState.claimedHalf ? (
                        <div className="bg-green-500/10 border border-green-500/20 text-green-400 py-1.5 rounded-xl text-[8px] font-black uppercase text-center font-mono">
                          Claimed ✓
                        </div>
                        ...
                        <button
                          ...
                          className="w-full bg-amber-400 text-black py-1.5 rounded-xl text-[8px] font-black uppercase text-center hover:bg-amber-300 transition-colors cursor-pointer border-0"
                        >
                          {claimingId === 'sit-half' ? 'Claiming...' : 'Claim'}
                        </button>
                      ) : (
                        <div className="bg-zinc-900 border border-zinc-805 text-zinc-650 py-1.5 rounded-xl text-[8px] font-bold uppercase text-center font-mono">
                  ...
                  <div className="p-4 bg-black/30 border border-zinc-900 rounded-2xl flex flex-col justify-between text-left">
                    <div>
                      <span className="text-[8px] font-black text-indigo-400 uppercase tracking-widest font-sans animate-pulse">2 Hour (Full)</span>
                      <p className="text-[10px] font-medium text-zinc-400 mt-1 uppercase font-mono">+{isNewUser ? "10,000,000" : "5,000,000"} Coins</p>
                    </div>
                    <div className="mt-3">
          ...
          {tasks.filter(t => filter === 'all' || t.status === filter).map(task => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <h3 className="text-white font-black uppercase text-sm tracking-tight">{task.title}</h3>
                    {task.id === 'engagement' && (
                      <span className={cn(
                        "text-[8px] font-black px-2 py-0.5 rounded-full border",
                        isNewUser ? "text-amber-400 border-amber-400/20" : "text-blue-400 border-blue-400/20"
                  ...
                  <p className="text-zinc-500 text-[10px] font-medium leading-relaxed max-w-[200px] font-sans">{task.description}</p>
                </div>
                <div className="text-right">
                  <div className="flex items-center justify-end gap-1 text-amber-400">
                  ...
                  <span className="text-zinc-600 text-[8px] font-black uppercase tracking-widest font-sans">COINS</span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                {task.id === 'verification' && task.status === 'available' && task.progress < task.target && (
                  <div className="space-y-4">
                    <button 
                      type="button"
                      onClick={() => setIsFaceModalOpen(true)}
                      className="w-full bg-white hover:bg-amber-400 text-black font-black py-4 rounded-2xl uppercase tracking-widest text-[10px] transition-all flex items-center justify-center gap-2 cursor-pointer border-0"
                    >
                      <Camera className="w-4 h-4" />
                ...
                {task.status !== 'completed' && task.id !== 'verification' && task.id !== 'daily-bonus' && (
                  <div className="flex items-center gap-2 mt-2">
                    <input
                      ...
                      className="flex-1 bg-black/40 border border-zinc-800 rounded-xl p-2 text-white text-[10px] w-20 outline-none"
                    />
                ...
                {task.id === 'engagement' && task.status === 'available' && (task.progress < task.target || task.progress === undefined) && (
                  <div className="w-full bg-zinc-800 text-zinc-400 font-black py-4 rounded-2xl uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 font-mono select-none">
                ...
                {task.status === 'available' && (task.progress >= task.target || task.id === 'daily-bonus') && (
                  <button 
                    ...
                    className="w-full bg-gradient-to-r from-green-500 to-emerald-500 text-white font-black py-4 rounded-2xl shadow-xl shadow-green-500/20 active:scale-95 transition-all uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 cursor-pointer border-0"
                  >
                    <Coins className="w-4 h-4" />
                ...
                {task.status === 'completed' && (
                  <div className="flex items-center gap-2 text-green-500 text-[9px] font-black uppercase tracking-widest font-sans">
                    <CheckCircle2 className="w-3 h-3 animate-ping shrink-0" />
                    Reward Credited ✓
                  </div>
                )}
              </div>
      ...
      {/* Info Card */}
      <div className="bg-blue-400/5 border border-blue-400/10 p-6 rounded-[2.5rem] flex items-start gap-4 text-left">
        <div className="bg-blue-400/20 p-2 rounded-xl text-blue-400 mt-1">
          <Info className="w-4 h-4" />
        </div>
        <div className="space-y-1" id="earning-rules">
          <p className="text-white text-xs font-bold uppercase tracking-tight font-sans">Earning Rules</p>
          <p className="text-zinc-500 text-[9px] font-medium leading-relaxed italic font-mono uppercase">
      ...
      <section className="space-y-6 pt-8 border-t border-zinc-900 text-left" id="host-earnings">
        <header className="flex items-center justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-green-400" />
              <span className="text-green-400 text-[10px] font-black uppercase tracking-[0.2em] font-sans">Host Revenue</span>
            </div>
          ...
          <div className="flex gap-2">
            <button 
              ...
              className={cn(
                "px-3 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all cursor-pointer",
        ...
        <div className="grid grid-cols-1 gap-4">
          {transactions.sort((a, b) => {
            ...
            <div key={earning.id} className="bg-zinc-900 border border-zinc-800 p-5 rounded-3xl flex items-center justify-between group hover:border-zinc-700 transition-all text-left">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-zinc-500 text-[8px] font-black uppercase tracking-widest font-sans">{earning.type?.replace('_', ' ')}</p>
                </div>
              </div>
              <div className="text-right">
                ...
                <p className="text-zinc-650 text-[8px] font-bold uppercase font-mono">{earning.date?.toLocaleDateString()}</p>
              </div>
            </div>
          ))}
          {transactions.length === 0 && (
            <div className="text-center py-12 bg-zinc-900/10 rounded-[2.5rem] border border-dashed border-zinc-800">
               <p className="text-zinc-600 text-[10px] font-black uppercase tracking-widest font-sans">No transactions found</p>
            </div>
          )}
        </div>
      </section>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="w-16 h-16 bg-gradient-to-br from-amber-400 to-orange-500 rounded-full flex items-center justify-center shadow-lg shadow-orange-500/20">
                <Coins className="w-8 h-8 text-black animate-bounce" />
              </div>
              <div className="space-y-2">
                <h3 className="text-2xl font-black text-white italic uppercase tracking-tighter">Claim Reward?</h3>
                <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest leading-relaxed font-sans">
              ...
              <div className="flex bg-black/40 border border-zinc-800 rounded-2xl px-6 py-3 items-center gap-2">
                <span className="text-zinc-500 text-[10px] font-black uppercase tracking-widest font-sans">Reward:</span>
              ...
              <div className="flex gap-3 w-full">
                <button
                  onClick={() => setTaskToConfirm(null)}
                  className="flex-1 bg-zinc-800 text-white font-black py-4 rounded-2xl uppercase tracking-widest text-[10px] hover:bg-zinc-700 transition-all cursor-pointer border-0"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 bg-gradient-to-r from-green-500 to-emerald-500 text-white font-black py-4 rounded-2xl shadow-xl shadow-green-500/20 active:scale-95 transition-all uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 cursor-pointer border-0"
                >
